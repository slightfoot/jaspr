...
class CreateCommand extends JasprCommand {
  CreateCommand();
  ...
  Future<int> runCommand() async {
    ...
    var process = await Process.start(
        'dart', ['create', '-t', 'web-simple', '--no-pub', directory.absolute.path]);

    pipeProcess(process, until: (s) => s.contains('Created project'));
    ...
    process =
        await Process.start('dart', ['pub', 'get'], workingDirectory: directory.absolute.path);

    pipeProcess(process, hide: (s) => s.contains('+'));
    await maybeExit(process);
